...
export function createIDFromEmail(email) {
  let input = shiftLetters(email.split("@")[0], (email.length % 2) + 1);
  let fragments = [];
  fragments.push(
    getWord(
      cutList(
        input.length % 3 === 0 ? strangeWords : [...strangeWords].reverse(),
        Math.floor(strangeWords.length / input.length)
      ),
      input
    )
  );
  fragments.push(
    getWord(
      cutList(
        [...normalWords].reverse(),
        Math.floor(strangeWords.length / input.length) % 4
      ),
      reverseString(input)
    )
  );
...
function makeNumber(letter, shift) {
  return letter << shift;
}
...
function cutList(list, index) {
  return [
    ...
    ...list.slice(index + 1, list.length - 1),
  ];
}
